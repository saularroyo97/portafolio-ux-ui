...
import { aboutData, toolsData } from "./data/aboutData";
...
{selectedCard?.content?.type === 'about' && (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div className="bg-white w-full max-w-5xl max-h-[90vh] rounded-2xl shadow-xl flex flex-col">
      ...
      {/* Header */}
      <div className="sticky top-0 z-10 bg-white p-6 border-b border-neutral-200 rounded-t-2xl flex justify-between items-center">
        <h2 className="text-3xl font-bold text-neutral-900">Sobre Mí</h2>
        <button onClick={closeModal} className="p-2 hover:bg-neutral-100 rounded-full transition-all">
          ✕
        </button>
      </div>
      ...
      {/* Contenido combinado */}
      <div className="overflow-y-auto p-6 space-y-16">
        
        {/* Sección Original (Biografía) */}
        {aboutData.map((section, index) => (
          <div key={`bio-${index}`} className="flex flex-col md:flex-row gap-8 items-start">
        ...
        {/* Sección Nueva (Herramientas estilo Stacky) */}

        {/* Sección de Habilidades */}
<div className="space-y-12">
  <h3 className="text-2xl font-bold text-center text-neutral-900">Skills & Tools</h3>
    ...
    {toolsData.map((section, index) => (
      ...
      >
        <h4 className="text-lg font-semibold mb-4 flex items-center gap-2">
          <span className="text-blue-600">▹</span> {/* Icono personalizado */}
          {section.title}
        </h4>
